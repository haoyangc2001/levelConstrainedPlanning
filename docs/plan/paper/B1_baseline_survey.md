...
|---|---|---|---|---|---|
| **OMPL** ConstrainedStateSpace / ProjectedStateSpace | projection (manifold) | **Yes**, Python wheel 2.0.1 verified locally | BSD-3-Clause (wheel metadata verified 2026-07-18) | **Med.** URDF→FK via our kinematics; `c(q)` as `ompl.base.Constraint.function`; world via our collision callback as a `StateValidityChecker`. Python bindings exist. | **Borrow for B5** (primary) |
