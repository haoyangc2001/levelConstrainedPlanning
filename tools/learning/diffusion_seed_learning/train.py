...
def main() -> None:
    ...
    model = TemporalUNet1D(dataset.dof, dataset.condition_dim, hidden_dim=args.hidden_dim).to(device)
    diffusion = GaussianDiffusion1D(args.diffusion_steps).to(device)
